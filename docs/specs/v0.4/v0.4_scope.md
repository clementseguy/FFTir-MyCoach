...
3. Refactoring sans régression
   ...
   - **🔴 CRITIQUE** : **Modèles dupliqués** - `lib/models.dart` contient des classes obsolètes dupliquées avec `lib/models/*.dart` (ShootingSession, Series)
   - **🟡 EN COURS** : Fichiers > 500 lignes à découper :
     - `exercises_list_screen.dart` (773L) - Extraire formulaire d'édition
     - `session_form.dart` (663L) - Modulariser en sous-composants  
     - `planned_session_wizard.dart` (572L) - Séparer étapes du wizard
     - `session_detail_screen.dart` (556L) - Extraire sections en composants
   - **🟡 À FAIRE** : Analyse statique pour supprimer composants non utilisés
   - **🟡 À FAIRE** : Tests unitaires pour écrans volumineux après refactor
...
## Dashboard (Améliorations Complétées)
